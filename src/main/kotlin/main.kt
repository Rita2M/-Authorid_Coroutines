        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            val postAu = PostWithAuthor(post,getAuthor(client, post.authorId))
                            val com = getComments(client, post.id)
                            val autCom =  com.map {comment ->
                                CommentsWithAuthor(getAuthor(client, comment.authorId), comment)
                            }
                           PostWithAuthorAndWithComments(postAu,autCom)
